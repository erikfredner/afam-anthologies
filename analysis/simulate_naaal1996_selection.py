"""
simulate_naaal1996_selection.py
-------------------------------
Trains logistic regression + Bayesian (Laplace) models on pre-1996
anthology appearances to predict NAAAL 1996 inclusion.

Produces:
  • console model comparison table
  • viz/naaal1996_predicted_prob_curves.png
  • top-N surprising absences tables (stdout; optionally CSV with --save-csv)

Usage:
    uv run python analysis/simulate_naaal1996_selection.py
    uv run python analysis/simulate_naaal1996_selection.py --only-root-works
    uv run python analysis/simulate_naaal1996_selection.py --save-csv
"""
...
FIG_OUT = Path(__file__).parent.parent / "viz" / "naaal1996_predicted_prob_curves.png"
FIG_OUT_LR = Path(__file__).parent.parent / "viz" / "naaal1996_logit_curves.png"
CSV_OUT_W = (
    Path(__file__).parent.parent / "data" / "naaal1996_surprising_absences_works.csv"
)
CSV_OUT_A = (
    Path(__file__).parent.parent / "data" / "naaal1996_surprising_absences_authors.csv"
)
...
def plot_probability_curves(
    ...
    ax.set_xlabel(
        "Number of pre-1996 non-NAAAL anthologies selecting item", fontsize=11
    )
    ax.set_ylabel("Predicted P(included in NAAAL 1996)", fontsize=11)
    ax.set_title(
...
def plot_logit_curves(
    ...
    ax.set_xlabel("Prior anthology appearances (non-NAAAL, ≤1996)", fontsize=11)
    ax.set_ylabel("P(selected for NAAAL 1996)", fontsize=11)
    ax.set_title(
        "Predicted probability of NAAAL 1996 inclusion by prior appearances",
        fontsize=12,
    )
...
def build_surprising_absences(
    ...
    """Items NOT in NAAAL, sorted by predicted P descending."""
    ...
    return absent[cols].head(top_n).reset_index(drop=True)
...
def print_surprising_absences(
    ...
    print(f"\n===== TOP {len(works_df)} SURPRISING ABSENCES — WORKS =====")
    print(works_df.to_string(index=False))

    print(f"\n===== TOP {len(authors_df)} SURPRISING ABSENCES — AUTHORS =====")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Simulate NAAAL 1996 selection with logistic + Bayesian models"
    )
    parser.add_argument(
        "--only-root-works",
        action="store_true",
    ...
    print("\n===== DATA OVERVIEW =====")
    print(f"Pre-1996 non-NAAAL editions counted : {n_pre_editions}")
    ...
    print(f"Works in NAAAL 1996                 : {int(work_frame['in_naaal'].sum())}")
    print(
        f"Authors in NAAAL 1996               : {int(author_frame['in_naaal'].sum())}"
    )
    ...
    print_surprising_absences(sa_works, sa_authors, save_csv=args.save_csv)
