...
def count_anthologies(input_csv):
    ...
    with open(input_csv, newline='', encoding='utf-8') as f:
        ...
        for col in ('series_name', 'anthology_name', 'anthology_year', 'anthology_edition_number'):
        ...
        for row in reader:
            ...
            author_key = (row.get('author_id', '').strip(), author_name) if use_id else (None, author_name)
...
def main():
    ...
    header = ['author_id', 'author_name', 'anthology_count'] if has_id else ['author_name', 'anthology_count']
    # Write output CSV
    with open(args.output_csv, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        # Sort by author_name
        for (author_id, author_name), count in sorted(counts.items(), key=lambda x: x[0][1]):
            if has_id:
