...
DATA_FILE  = Path(__file__).parent.parent / "data" / "2026-03-13 works per afam anthology.csv"
OUT_FILE   = Path(__file__).parent / "naaal1996_prior_selection.png"
OUT_FILE_2 = Path(__file__).parent / "naaal1996_prior_selection_twopanel.png"
...
def plot(
    work_ks:   np.ndarray, work_pcts:   np.ndarray, work_ns:   np.ndarray, n_works:   int,
    auth_ks:   np.ndarray, auth_pcts:   np.ndarray, auth_ns:   np.ndarray, n_authors: int,
    out: Path,
) -> None:
    ...
    ax.scatter(work_ks, work_pcts, s=np.sqrt(work_ns) * 10,
               color=WORK_COLOR,   marker="o", alpha=0.6, zorder=3, label=work_label)

    ax.plot(auth_ks, auth_pcts, color=AUTHOR_COLOR, linewidth=0.8, alpha=0.4, zorder=2)
    ax.scatter(auth_ks, auth_pcts, s=np.sqrt(auth_ns) * 10,
               color=AUTHOR_COLOR, marker="s", alpha=0.6, zorder=3, label=author_label)
...
def plot_two_panel(
    work_ks, work_pcts, work_ns, n_works,
    auth_ks, auth_pcts, auth_ns, n_authors,
    out: Path,
) -> None:
    work_label   = f"Works (N={n_works:,})"
    author_label = f"Authors (N={n_authors:,})"
    ...
    ax_pct.plot(work_ks, work_pcts, color=WORK_COLOR,   linewidth=1.2, alpha=0.5, zorder=2)
    ax_pct.scatter(work_ks, work_pcts, s=40, color=WORK_COLOR,   marker="o", alpha=0.6, zorder=3, label=work_label)
    ax_pct.plot(auth_ks, auth_pcts, color=AUTHOR_COLOR, linewidth=1.2, alpha=0.5, zorder=2)
    ax_pct.scatter(auth_ks, auth_pcts, s=40, color=AUTHOR_COLOR, marker="s", alpha=0.6, zorder=3, label=author_label)

    ax_pct.set_ylabel(
        "% selected for the 1996 Norton Anthology of\nAfrican American Literature", fontsize=11)
    ax_pct.set_title(
        "Prior anthology selection as a predictor of inclusion in NAAAL 1996",
        fontsize=12)
    ...
    bar_width = 0.35
    ax_n.bar(work_ks - bar_width / 2, work_ns, width=bar_width, color=WORK_COLOR,   alpha=0.7)
    ax_n.bar(auth_ks + bar_width / 2, auth_ns, width=bar_width, color=AUTHOR_COLOR, alpha=0.7)
    ax_n.set_yscale("log")
...
def main() -> None:
    ...
    plot(work_ks, work_pcts, work_ns, len(work_frame),
         auth_ks, auth_pcts, auth_ns, len(author_frame),
         OUT_FILE)
    plot_two_panel(work_ks, work_pcts, work_ns, len(work_frame),
                   auth_ks, auth_pcts, auth_ns, len(author_frame),
                   OUT_FILE_2)
