...
def main():
    ...
    try:
        ...
        raise ValueError("Could not uniquely identify full vs. Shorter Edition series names.")
    ...
    for ed in all_editions:
        rows.append({
            ...
            'NAAL reselected %': full_summary.get(ed, {}).get('reselected_pct', 0.0),
            'NAALSE reselected': shorter_summary.get(ed, {}).get('reselected', 0),
            'NAALSE reselected %': shorter_summary.get(ed, {}).get('reselected_pct', 0.0),
        })
