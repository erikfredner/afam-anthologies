...
# Rebuild the bibliography pages in docs/ (both need pandoc on PATH)
...
| `reselection/` | `reselection_probability`, `work_reselection_probability`, `gender_reselection`, `cumulative_pairwise_agreement`, `series_pair_reselection`, `author_selection_spread`, `author_page_share_reselection`, `retention_from_1929`, `retention_from_1941`, `first_selection_success`, `continuation_probability`, `edition_pair_retention_scatter`, `debut_reselection_forest`, `frequent_author_work_selection_scatter`, `reselection_hazard_curves` | Per-author/work reselection and retention trends; `reselection_hazard_curves` plots survival S(t), discrete hazard h(t), and the hazard-ratio forest from `analysis/reselection/reselection_hazard`; `edition_pair_retention_scatter` takes `--authored-works-only`, which drops works with no author on record (298 of 2,673) and writes the variant to `edition_pair_retention_scatter_authored_only.*` — unauthored works can never contribute to author retention, so this restricts both axes to the same authored material (author retention exceeds work retention in 311/311 chronological pairs vs. 310/311 in the default scope), and also writes an interactive plotly version to `docs/edition_pair_retention_scatter.html` (tracked, served by GitHub Pages; `--html-out` overrides the path) where hovering any of the 339 points names both anthologies, the direction, and the counts behind each share — so it carries no `CALLOUT_PAIRS` annotations, which exist in the static figure only because it can label seven pairs at most. That HTML is a full page, not a bare plotly dump: `plot_html` wraps the chart (`to_html(full_html=False)`, responsive width, a fixed `div_id` so a regeneration does not churn a random UUID through a tracked file) in the site's house style plus the reading context from the module docstring — what a point is, what the diagonal means, why same-series and same-year pairs are marked apart. Every number in that prose comes from `page_summary()`, a pure helper over the plotted frame, so the text cannot drift from the figure; `_exception_sentence` names the chronological pairs that do *not* sit above the line rather than leaving them a residual. The chart panel keeps its own white ground because plotly's `plotly_white` template ignores `prefers-color-scheme`. The HTML is written for the default scope only, and loads plotly.js from the CDN rather than inlining it, since `docs/` is committed |
...
| Script | Purpose |
|---|---|
| `bibliography.py` | Shared pandoc/citeproc plumbing for the two bibliography pages, imported as a `scripts/` sibling (`sys.path[0]` when a script there is run, and the directory the tests already insert): `bib_keys`, `nocite_metadata` (`@*` or an explicit key list), `render_bibliography`, `assert_all_rendered`, `retag_bib_div`, `require_files`, and the `PAGE_CSS` both pages share with `docs/index.html` |
...
| `build_comprehensive_bibliography.py` | Renders the two comprehensive `.bib` files into `docs/comprehensive_bibliography.html` as two Chicago sections — **In the analysis** (26) and **Considered, not included** (13). Both sections are rendered from `comprehensive_all.bib` alone, via an explicit `nocite` key list per section; `comprehensive_selected.bib` is read only for its keys, since the shared records are identical between the files. `split_keys` exits if a selected key is missing from the full list (it would silently vanish from the page), and each section's rendered count is checked against the keys requested. The page states only that the 13 were eligible but are not part of the analysis — no per-title rationale, and no claim about why any one was set aside. `--all-bib`, `--selected-bib`, `--csl`, `--out` |
| `build_selection_tables.py` | Live DB pull → `docs/author_selection_records.html` (all 575 authors) and `docs/work_selection_records.html` (all 3,236 works), reading `queries/authors-anthology-record.sql` and `queries/works-anthology-record.sql`. Both records are rendered as percentages — share of the 26 anthologies selected, and share of post-debut anthologies that reselected — plus a third **vs. peers** column giving that second figure as a signed integer relative to the entity's debut cohort. A raw reselection rate is not comparable across rows, since the denominator is set by debut date (a 1929 debut has had 25 chances, a 2014 debut one), so `add_peer_delta` groups on the new `debut_edition_id` and scores each entity in percentage points against the *other* entities that debuted in the same anthology — a leave-one-out baseline (the focal row is removed from its own comparison, as in `analysis/reselection/post_debut_performance.py`) reported in the percentage-point unit `analysis/reselection/reselection_vs_chance.py` uses. Cohort members all share one `opportunities` count, so the pooled peer rate and the mean of peers' rates coincide, and each cohort's deltas sum to exactly zero. Both the reselection and vs.-peers cells are blank for the 20 authors and 70 works that debuted in the 2025 *NAAAL* and have had no opportunity; vs. peers would also blank a lone-member cohort, of which there are currently none. Works count excerpts in their own right, with the parent volume named in an `In` column; angle-bracket title qualifiers (`The New Negro <essay>`) are split off and rendered as a muted parenthetical rather than dropped, since they are what distinguishes Locke's essay from the collection it opens. Every column carries a filter box under its heading, rendered from a `Column` spec (`filter` `auto`/`text`/`range`/`none`, plus the range placeholders): a substring match for text columns, a lowest/highest pair for numeric ones, and an explicit `range` on **Dates**, which is not a numeric column but is most usefully filtered by year. The pair compares against what the cell *displays* — the rounded percent, the signed percentage points, the year a date cell leads with (`"b. 1944"` → 1944) — because that is what the reader types, while `data-sort` keeps the exact unrounded value for the sort, whose units need not match the display (**Selected** shows a percent but ranks on the count). A cell marked `data-missing` fails any range, on the same reasoning that sinks it in both directions of a sort: no opportunity is not a low rate. The script rebuilds the `tbody` from the passing rows rather than hiding the rest, so the `nth-child` zebra striping stays alternating, and it sorts the full row set so clearing a filter restores the hidden rows in the reader's chosen order. A `.tablemeta` line above the table counts what is showing (`data-unit` names the noun) and offers a clear-filters button. The 298 works with no author on record keep an **empty** Author cell — `join_author_names` here deliberately differs from `analysis/overlap/works_in_half_or_more_afam_eds.join_authors`, which credits them to "Anonymous"; the absence of attribution is itself evidence and must not be imputed. `--only {authors,works,both}`, `--authors-out`, `--works-out` |

The generated pages are plain hand-editable HTML — no template engine, no CDN, the only script
is ~100 lines of inline column sorting and filtering — so prose and individual rows can be
corrected by hand and the edits read as ordinary git diffs. **Re-running a generator overwrites those edits.**
...
`authors-anthology-record.sql` and `works-anthology-record.sql` are the full selection records — every author (575) and every work (3,236) ever selected, with debut-relative reselection counts. They are the `*-in-half-or-more-afam-eds.sql` queries minus the `edition_count * 2 >= total_editions` threshold, plus a `debut_edition_id` column — the anthology the entity debuted in, ties within a year broken by lowest edition id, matching the repo-wide `(year, edition_id)` debut ordering — which is the cohort key behind the pages' **vs. peers** column. They back the `docs/` selection-record pages; keep the four in sync if the reselection logic changes (`debut_edition_id` adds a column without touching that logic, so the half-or-more pair did not need it).
...
| `data/comprehensive_all.bib` | `scripts/build_comprehensive_bibliography.py` — the 39 anthologies eligible for consideration in the project; the single source of every rendered entry on the comprehensive page |
| `data/comprehensive_selected.bib` | `scripts/build_comprehensive_bibliography.py` — the 26 of those that were indexed and analyzed. A strict subset of `comprehensive_all.bib`, with byte-identical records, so it is read only for its citation keys |
| `chicago-notes-bibliography.csl` | `scripts/build_binder_bibliography.py`, `scripts/build_comprehensive_bibliography.py` — CSL style passed to pandoc's citeproc |
