...
| `vernacular/` | `vernacular_share`, `vernacular_reselection_forest` | Bar chart of each edition's vernacular share, by selection count and by page extent; forest plot of the eight `vernacular_reselection_test` estimates — observed vernacular rate (95% Wilson CI) over each Monte Carlo null's 95% interval, per metric × sampling mode |
